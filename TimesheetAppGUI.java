        ...
        endTaskButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (!taskName.isEmpty()) {
                    ...
                    // Find the task and display its duration
                    Task task = null;
                    for (Task t : user.getTimeSheet().getTasks()) {
                        if (t.getTaskName().equals(taskName) && !t.isOngoing()) {
                            task = t;
                            break;
                        }
                    }
                    if (task != null) {
                        Duration duration = task.getTaskDuration();
                        taskArea.append("\nEnded task: " + taskName + ", Duration: " + duration.toMinutes() + " minutes");
                    } else {
                        taskArea.append("\nNo ongoing task with that name.");
                    }
